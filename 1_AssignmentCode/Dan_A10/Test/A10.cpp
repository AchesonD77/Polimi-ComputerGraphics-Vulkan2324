// T       his has been adapted from the Vulkan tutorial
...
struct EarthMatricesUniformBufferObject {
    ...
    alignas(16) glm::mat4 nMat;    // normal
};


// **A10** Place here the CPP struct for the uniform buffer for the parameters
struct EarthParamsUniformBufferObject {
    alignas(4) float Pow;
    alignas(4) float Ang;
    alignas(4) float ShowCloud;    // for could
    alignas(4) float ShowTexture;  //  for texture
};
...
protected:
    ...
    // **A10** 
    // Place here the variables for the Model, the five texture (diffuse, specular, normal map, emission and clouds) and the Descrptor Set
    Model MEarth;
    ...
    {6, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, VK_SHADER_STAGE_FRAGMENT_BIT, sizeof(EarthParamsUniformBufferObject), 1 }
            });
    ...
    void localCleanup() {
        ...
        TEarthDiffuse.cleanup();  // clean Diffuse
        TEarthSpecular.cleanup();  // clean Specular
        TEarthNormal.cleanup();  // clean Normal Map
        TEarthEmission.cleanup();  // clean Emission
        TEarthClouds.cleanup();  // clean Cloud
        MEarth.cleanup();  // clean model
    ...
    void updateUniformBuffer(uint32_t currentImage) {
        ...
        if (currScene == 1) {
            ...
            if (subpassTimer > 4.0f) {
                ...
                char buf[20];
                sprintf(buf, "A10_%d.png", subpass); // in my xcode, i need to change sprintf_s to sprintf.
        ...
        EarthMatricesUniformBufferObject earthUbo{};
        earthUbo.mvpMat = ViewPrj;
        earthUbo.mMat = glm::mat4(1.0f);
        earthUbo.nMat = glm::inverse(glm::transpose(earthUbo.mMat));
        DSEarth.map(currentImage, &earthUbo, 0);

        EarthParamsUniformBufferObject earthParamsUbo{};
        earthParamsUbo.Pow = 200.0f;
        earthParamsUbo.Ang = tTime * TangTurnTimeFact;
        earthParamsUbo.ShowCloud = ShowCloud;
        earthParamsUbo.ShowTexture = ShowTexture;
        DSEarth.map(currentImage, &earthParamsUbo, 6);
