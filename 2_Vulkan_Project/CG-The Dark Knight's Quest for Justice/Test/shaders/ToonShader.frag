...
// devide the light levels
float toonShadingStep(float intensity, float levels) {
...
void main() {
    ...
    float toonDiffuse = toonShadingStep(diffuseIntensity, 4.0); // devide into 3 levels

    // specular reflection
    vec3 halfwayDir = normalize(lightDir + EyeDir);
    float specularIntensity = pow(max(dot(Norm, halfwayDir), 0.0), 50.0f);
    float toonSpecular = toonShadingStep(specularIntensity, 3.0); // devide into 3 levels

    // ambient light
    vec3 Ambient = texture(tex, fragUV).rgb * 0.05;

    // Cartoon lighting effect
    vec3 Diffuse = texture(tex, fragUV).rgb * toonDiffuse * 0.5f;
    vec3 Specular = vec3(toonSpecular);
